...
> 翻译自 [You Might Not Need an Effect](https://beta-reactjs-org-git-you-might-not-fbopensource.vercel.app/learn/you-might-not-need-an-effect) - [React beta Doc](https://beta-reactjs-org-git-you-might-not-fbopensource.vercel.app/)
...
function ProfilePage({ userId }) {
  ...
  // 🔴 Avoid: 在 Effect 里重置 state 当 props 改变
  useEffect(() => {
    setComment('');
  }, [userId]);
  // ...
...
这么做是效率很低的，`ProfilePage` 和 `children` 组件先根据旧的状态进行渲染，然后再根据新的状态再来一遍整个过程。更复杂的是你需要在 `ProfilePage` 的每个需要这个 state 的子组件中这么做。例如，如果评论组件是嵌套的，你需要清空所有的嵌套组件的 `commnet` 状态。
...
React 通常会保留同一个组件在同一个位置渲染时的状态。**通过将 `userId` 做为 `key` 传递给 `Profile`，告诉 React 不同的 `userId` 对应的 `Profile` 是不同的组件，它们之间不应该共享任何状态**。当 `key`（这里指 `userId`）改变时，React 将重建 DOM 并重置 `Profile` 以及其子组件的所有状态，因此在 `Profile` 切换时，`commnet` 组件的状态会*自动*被清除。

### props 更改时更改某些 state

有时候你可能不想在 props 改变时重置所有 state，而只是部分。

`List` 组件接收 `items` 属性，并且持有被选择项的 state `selection`，你可能想在 `items` 变动时将 `selection` 清除：
...
每次 `items` 改变，`List` 和其子组件先根据旧的 `selection` 值渲染，然后 React 更新 DOM，执行 Effect，`setSelection(null)` 执行，导致 `List` 和其子组件重新渲染，重复上面的整个过程。

删除 Effect，在渲染期间更新 state：
...
那么有没有更简单在渲染期间计算某些值的办法呢？例如，不存储（重置）被选择项，只存储被选择项的 ID：
...
现在根本不需要”调整“状态。如果被选择项的 ID 在 list 中，仍然会被选择。如果不存在，`selection` 则会在渲染时计算，结果将会是 `null`。This behavior is a bit different, but arguably it’s better because most changes to items now preserve the selection. However, you’d need to use `selection` in all the logic below because an item with `selectedId` might not exist.
...
这里的 Effect 是没什么必要的，而且容易产生 bugs。例如假设你的应用在页面重新加载后“记住”了购物车。如果你把商品加入到购物车然后刷新页面，则会再次弹出加入购物车的提示信息，而且每次刷新页面都会提示。很显然 `product.isInCart` 在页面加载后是 `true`，所以 Effect 里的 `showToast()` 被调用。
...
因为分析事件需要在表单被展示给用户后发送，所以应该在 Effect 里。（它会在开发环境中触发两次，请参阅[此处](https://beta-reactjs-org-git-you-might-not-fbopensource.vercel.app/learn/synchronizing-with-effects#sending-analytics)了解如何处理。）

然而，发送给 `/api/register` 的请求不是在表单被展示给用户触发。你只想在一个具体的时间（用户点击提交按钮）发送。删除第二个 Effect，在表单提交的事件里发送请求：
...
当您选择是否将某些逻辑放入 event handler 或 Effect 时，您需要回答的主要问题是从用户的角度来看它是哪种逻辑。如果此逻辑是由特定交互引起的，请在 event handler 中处理。如果它是由用户在屏幕上看到组件引起的，请 Effect 中处理。
...
和前面类似，`Toggle` 先更新 state，然后 React 更新视图。React 执行 Effect 里父组件传递 `onChange` 事件回调。然后父组件更新自己的 state，又一次渲染过程。最好在一次渲染过程中做完所有这些。
...
你也可以移除 `Toggle` 内部的 `isOn` state，通过父组件传递下来：
...
[“状态提升”](https://beta-reactjs-org-git-you-might-not-fbopensource.vercel.app/learn/sharing-state-between-components) 让父组件根据自己的状态控制 `Toggle` 的状态。这也意味着父组件必须包含更多的逻辑，但是总体需要关心的状态减少了。当你试图将两个状态保持同步时，是尝试状态提升的信号！
...
这让数据流向变得简单，以及更可预测：数据从上（父）到下（子）流动。

### 订阅外部状态

有的时候，你的组件可能需要订阅 React 状态之外的数据状态。这些数据可能来值第三方库或者浏览器 API。由于这些数据可能会在 React 不知情的情况下更新，因此通常需要手动在组件内通过 Effect 监听这些更新：
...
组件订阅了外部数据源（这里是浏览器的 `navigator.onLine` API）。由于该 API 在服务端是不存在的（因此不能用来生产初始 HTML），初始状态设置为 `true`。每当浏览器中该数据的值改变时，组件会更新状态。
...
这似乎和之前需要将逻辑放到 event handler 的例子相矛盾！但是，考虑用户输入不是获取数据主要原因，搜索关键字通常通过 URL 预填，用户可以在不输入的情况下向前或向后导航，但是你希望页面和查询结果和远程的数据保持同步，这就是为什么使用 Effect。

然而，上面的代码有一个 bug。想象你快速的打出 `"hello"`，然后 `query` 会从 `"h"`，`"he"`，`"hel"`，`"hell"` 最后变到 `"hello"`。这会发出几个请求，但不能保证结果按什么顺序返回。例如 `"hell"`的响应在 `"hello"` 的响应返回后返回。因此 `"hell"` 的结果会最后 set 给 `results`，展示了错误的搜索结果。这叫做[”竞态“](https://en.wikipedia.org/wiki/Race_condition)：两个不同请求相互“竞争”，并以非预期顺序返回。
...
处理竞态并不是实现数据请求的唯一困难。你可能也想缓存结果（以便用户返回上个页面时不用重新加载数据），如何在服务端获取数据（以便生成服务端渲染的初始 HTML 时包含获取到的数据），如何避免 network waterfalls（以便子组件不必等到所有父组件获取完数据才能开始获取自己的数据）。这些问题不仅在 React 中存在，任何的 UI 库都有。解决它们也并非易事，这就是现代框架提供比直接在组件中编写更有效的内置数据获取机制的原因。

如果你不使用框架（并且不想构建自己的框架）但希望从 Effects 中获取数据更符合直觉，考虑将获取逻辑提取到自定义 Hook 中：
...
你可能想添加一些错误处理的逻辑和追踪内容是否正在加载。你可以构建自己的 Hook，也可以使用 React 生态中的众多解决方案之一。**尽管这不会和使用框架内置的数据获取机制那么高效，但是这些逻辑移动到自定义 Hook 后，后续更换更加高效的数据获取策略更方便**。
